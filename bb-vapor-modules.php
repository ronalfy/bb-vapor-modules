...
function bbvm_beaver_builder_module_init() {
	if ( class_exists( 'FLBuilder' ) ) {

		add_action( 'wp_head', 'bbvm_beaver_builder_ajax_url' );
		...
		add_shortcode( 'bbvm_bb_copyright', 'bbvm_beaver_builder_copyright' );
	}


}

function bbvm_beaver_builder_ajax_url() {
?>
<script>
...
function bbvm_beaver_builder_plugin_loaded() {
	// Load text domain
...
add_action( 'init', 'bbvm_beaver_builder_module_init', 20 );
add_action( 'plugin_loaded', 'bbvm_beaver_builder_plugin_loaded' );
require_once( 'includes/bbvm-beaverbuilder-admin.php' );
new BBVapor_BeaverBuilder_Admin();

function bbvm_beaver_builder_copyright( $atts ) {
